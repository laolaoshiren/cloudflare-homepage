...
router.get('/api/links', async (request, env) => {
    try {
        const { results } = await env.DB.prepare('SELECT * FROM links ORDER BY sort_order ASC, created_at ASC').all();
        // 确保返回空数组而不是 null
        return new Response(JSON.stringify(results || []), {
            headers: createCorsHeaders(request, env)
        });
...
router.post('/api/links', async (request, env) => {
    try {
        ...
        const result = await env.DB.prepare('INSERT INTO links (title, url, icon, sort_order) VALUES (?, ?, ?, ?) RETURNING id')
            .bind(link.title, link.url, link.icon || null, link.sort_order || 0)
            .first();
        return new Response(JSON.stringify({
            success: true,
            id: result?.id
...
router.delete('/api/links/:id', async (request, env) => {
    try {
        ...
        const { success } = await env.DB.prepare('DELETE FROM links WHERE id = ?')
            .bind(id)
            .run();
        return new Response(JSON.stringify({ success }), {
            headers: createCorsHeaders(request, env)
...
router.put('/api/links/:id', async (request, env) => {
    try {
        ...
        // 规范化 URL
        link.url = normalizeUrl(link.url);
        const { success } = await env.DB.prepare('UPDATE links SET title = ?, url = ?, icon = ?, sort_order = ? WHERE id = ?')
            .bind(link.title, link.url, link.icon || null, link.sort_order || 0, id)
            .run();
        return new Response(JSON.stringify({ success }), {
            headers: createCorsHeaders(request, env)
        });
...
// 获取联系方式列表
router.get('/api/contacts', async (request, env) => {
    try {
        const { results } = await env.DB.prepare('SELECT * FROM contacts WHERE is_public = ? ORDER BY sort_order ASC, created_at ASC')
            .bind(true)
            .all();
        return new Response(JSON.stringify(results || []), {
            headers: createCorsHeaders(request, env)
...
// 添加联系方式（需要管理员权限）
router.post('/api/contacts', async (request, env) => {
    try {
        await validateToken(request, env);
        ...
        const result = await env.DB.prepare('INSERT INTO contacts (type, value, icon, is_public, sort_order) VALUES (?, ?, ?, ?, ?) RETURNING id')
            .bind(contact.type, contact.value, contact.icon || null, contact.is_public, contact.sort_order || 0)
            .first();
        return new Response(JSON.stringify({
            success: true,
            id: result?.id
...
// 删除联系方式
router.delete('/api/contacts/:id', async (request, env) => {
    try {
        await validateToken(request, env);
        const { id } = request.params;
        const { success } = await env.DB.prepare('DELETE FROM contacts WHERE id = ?')
            .bind(id)
            .run();
        return new Response(JSON.stringify({ success }), {
            headers: createCorsHeaders(request, env)
...
// 处理未匹配的路由
router.all('*', (request, env) => new Response('Not Found', {
    status: 404,
    headers: createCorsHeaders(request, env)
}));
export default {
