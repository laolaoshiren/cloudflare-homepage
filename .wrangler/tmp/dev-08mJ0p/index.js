...
// .wrangler/tmp/bundle-9dvmnL/checked-fetch.js
...
router.post("/api/projects", async (request, env) => {
  ...
  const { success } = await env.DB.prepare("INSERT INTO projects (title, description, image_url, github_url, demo_url) VALUES (?, ?, ?, ?, ?)").bind(project.title, project.description || "", project.image_url || null, project.github_url || null, project.demo_url || null).run();
...
// .wrangler/tmp/bundle-9dvmnL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9dvmnL/middleware-loader.entry.ts
